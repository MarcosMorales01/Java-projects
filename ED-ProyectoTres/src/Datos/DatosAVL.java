 ...
 */
public class DatosAVL extends Datos {
    ArbolAVL Nombre = null;
    ArbolAVL Profesion = null;
    ArbolAVL Calificacion = null; 
    String Path;
    ...
    public DatosAVL(String path, boolean nom, boolean prof, boolean cal) throws IOException {
        Path = path;
        CampoNom = nom; 
        CampoProf = prof;
        CampoCal = cal;
    ...
    public void leer() throws IOException {
        ...
        if(CampoProf){
            Profesion = new ArbolAVL();
        }
        if(CampoCal){
            Calificacion = new ArbolAVL();
        }
                ...
                if(!(linea = buffer.readLine()).equals("\0")){
                   ...
                   if(CampoNom){
                       if(Nombre.getRaiz()==null){
                           NodoAVL aux = new NodoAVL(new Elemento(fHash(palabras[0]), i));
                           Nombre.setRaiz(aux);
                       }else{
                    ...
                    if (CampoProf) {
                        if (Profesion.getRaiz() == null) {
                            NodoAVL aux = new NodoAVL(new Elemento(fHash(palabras[1]), i));
                    ...
                    if (CampoCal) {
                        if (Calificacion.getRaiz() == null) {
                            NodoAVL aux = new NodoAVL(new Elemento(fHash(palabras[2]), i));
    ...
    public static void main(String[] arsg) throws IOException {
        DatosAVL d = new DatosAVL("hola", false, true, true);
        ArrayList a = d.Busqueda("", "Educacion", "75");
