...
public class NodoBin {
    ...
    public void inOrden(){ //IRD
       if(izq!=null)
           izq.inOrden();
     System.out.println(((Elemento)dato).getDato());//Raiz
       if(der!=null)
           der.inOrden();
    }
    
    public void Busqueda(int indice, ArrayList aux){
        if (izq != null) {
            izq.Busqueda(indice, aux);
        }
        if(indice == ((Elemento) dato).getDato()){
            aux.add(dato); 
        }
        if (der != null) {
            der.Busqueda(indice, aux);
        }
